...
import { setLogin, setProfileURL, setSpotifyAuth } from "state";
...
const Form = () => {
    ...
    const register = async (values, onSubmitProps) => {
        ...
        // looping over the values in setfieldvalue and adding them to formData
        for (let value in values) {
            formData.append(value, values[value]);
        }
        formData.append("picturePath", values.picture.name) // we already have the picture from setFieldValue, we may not need the picturePath, but rather an ID for it?
        const savedUserResponse = await fetch("http://localhost:3001/auth/register",
        {
            method: "POST",
            body: formData,
        }) // FETCH COMMAND TO CALL BACKEND API, our backend is hosted on port 3001, and we get path of method we need

        const savedUser = await savedUserResponse.json(); // might need to check if resposne is error or not
    ...
    const login = async (values, onSubmitProps) => {
        ...
        if (loggedIn){
            dispatch(
                setLogin({ // Login command we defined in state, to modify our redux state. We are passed back the user and token from the backend api we called the payload is the object we make with user and token attributes
                    user: loggedIn.user,
                    token: loggedIn.token
                })
                
            );
            // const redirectUrl = loggedIn.redirect
            // window.location.href = redirectUrl // redirect to spotify auth url which then redirects to homepage (might need to change)
            const response = await fetch("http://localhost:3001/auth/spotifyAuth", {
                method: "GET",
            })
            ...
            {({
                ...
                <form onSubmit={handleSubmit}>
                    <Box display="grid" gap="30px" gridTemplateColumns="repeat(4, minmax(0,1fr))" sx={{
                        ...
                        <TextField label="Password" type="password" onBlur={handleBlur}onChange={handleChange} value={values.password} name="password" error={Boolean(touched.password) 
                            && Boolean(errors.password)} helperText={touched.password && errors.password} sx={{ gridColumn: "span 4" }} />
                    </Box>

                    {/* BUTTONS */}
                    <Box>
                        <Button fullWidth type="submit" sx={{m: "2rem 0", padding: "1rem", backgroundColor:palette.primary.main, color:palette.background.alt, "&:hover": {color:palette.primary.main}}}>
